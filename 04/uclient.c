...
int main(int argc, char *argv[])
{
	...
	sendto(sock, data.buf, strlen(data.buf), 0, (struct sockaddr*)&serv_addr, sizeof(serv_addr));

	//파일이 있는 지 없는지 read
	// str_len = read(sock, message, sizeof(message));
	adr_sz = sizeof(from_adr);
	str_len = recvfrom(sock, message, strlen(message), 0, (struct sockaddr*)&from_adr, &adr_sz);
    ...
    if(!strcmp(message, "FileNotFound")){//서버로부터 파일이 없는 경우
    	...
    	while(1){
			...
			str_len = recvfrom(sock, &data, sizeof(data), 0, (struct sockaddr*)&from_adr, &adr_sz);
			...
			if(data.buf_len<100){ //파일 마지막 부분을 보낸 경우
				...
				sendto(sock, &data, sizeof(data), 0, (struct sockaddr*)&serv_addr, sizeof(serv_addr));
