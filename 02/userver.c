...
int main(int argc, char *argv[])
{
    ...
    str_len = recvfrom(serv_sock, data.buf, BUF_SIZE, 0, (struct sockaddr*)&clnt_addr, &clnt_addr_size);
	...
	if(file==-1){
    	...
    	while((data.buf_len = read(file, data.buf, BUF_SIZE))>=0)
		{
            ...
            sendto(serv_sock, &data, sizeof(data), 0, (struct sockaddr*)&clnt_addr, clnt_addr_size);
 
		
			//클라이언트로부터 변경된 data 받기
			// str_len = read(clnt_sock, &data, sizeof(data)); 
            str_len = recvfrom(serv_sock, &data, sizeof(data), 0, (struct sockaddr*)&clnt_addr, &clnt_addr_size);
			//보낸 후 데이터 리셋
			memset(data.buf, 0, BUF_SIZE);
